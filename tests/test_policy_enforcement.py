...
def test_get_token_ttl_clamped_by_policy(tmp_path: Path) -> None:
    runtime_dir = tmp_path / ".runtime"
    vault = VaultKnox(expand_runtime_path(runtime_dir))
    vault.initialize(STRONG_PASSWORD)
    vault.unlock(STRONG_PASSWORD)
...
def test_policy_resolves_encrypted_metadata_service(tmp_path: Path) -> None:
    runtime_dir = tmp_path / ".runtime"
    vault = VaultKnox(expand_runtime_path(runtime_dir))
    vault.initialize(STRONG_PASSWORD)
    vault.unlock(STRONG_PASSWORD)
    vault.add_secret(STRONG_PASSWORD, "api_1", "api_key", "OpenAI", {"key": "sk-test", "service": "openai"})

    policy = {
        "version": "2",
        "agents": {
            "agent-a": {
                "services": {"openai": {"actions": ["get_metadata"]}},
            }
        },
    }
    _write_policy(runtime_dir, policy)

    result = vault_tool("get_masked", runtime_dir=str(runtime_dir), agent_id="agent-a", secret_id="api_1")
