...
def install_hooks() -> None:
    """Install the VaultKnox Hermes plugin into ~/.hermes/plugins/vaultknox/.

    The plugin registers:

    - ``pre_gateway_dispatch``: redacts secrets from inbound messages
    ...
    Notes
    -----
    - Legacy ``~/.hermes/hooks/secret-guard/`` gateway hooks are no longer
      deployed: current Hermes does not consume hook-event write-backs, so
      they had no effect. The plugin replaces them.
    - A superseded ``vaultknox-secret-guard`` plugin directory is reported
      (never auto-deleted).
    """
    # --- Gateway plugin (packaged: vaultknox/_hermes_plugin) ---
    package_plugin_dir = Path(__file__).resolve().parent / "_hermes_plugin"
    plugin_dir = Path.home() / ".hermes" / "plugins" / "vaultknox"

    if not package_plugin_dir.is_dir():
        raise click.ClickException(
            f"Packaged plugin directory not found: {package_plugin_dir}. "
            "Reinstall the vaultknox package and retry."
        )

    plugin_dir.mkdir(parents=True, exist_ok=True)
    installed: list[str] = []
    for name in ("__init__.py", "detectors.py", "plugin.yaml"):
        src = package_plugin_dir / name
        if not src.is_file():
            raise click.ClickException(f"Packaged plugin file missing: {src}")
        shutil.copyfile(src, plugin_dir / name)
        installed.append(name)

    from vaultknox import __version__

    click.echo(f"  ✅ Installed vaultknox plugin (v{__version__}) to {plugin_dir}")
    for name in installed:
        click.echo(f"     • {name}")

    superseded_plugin = Path.home() / ".hermes" / "plugins" / "vaultknox-secret-guard"
    if superseded_plugin.is_dir():
        click.echo()
        click.echo(f"  ⚠️  Superseded plugin directory found: {superseded_plugin}")
        click.echo("     Disable 'vaultknox-secret-guard' in Hermes config.yaml and")
        click.echo("     remove that directory when ready; it is replaced by 'vaultknox'.")

    legacy_hook_dir = Path.home() / ".hermes" / "hooks" / "secret-guard"
    if legacy_hook_dir.is_dir():
        click.echo()
        click.echo(f"  ℹ️  Legacy hook directory present: {legacy_hook_dir}")
        click.echo("     No longer used or deployed; safe to remove.")

    click.echo()
    click.echo("  To activate, add to Hermes config.yaml:")
    click.echo("    plugins:")
    click.echo("      enabled:")
    click.echo("        - vaultknox")
